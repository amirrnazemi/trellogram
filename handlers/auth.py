...
async def show_boards(message: types.Message, user: User):
    ...
    for board in boards:
        keyboard.add(types.InlineKeyboardButton(text=board['name'], callback_data=f"show_lists:{board['id']}"))
...
async def show_lists(callback_query: types.CallbackQuery, user: User):
    ...
    keyboard = types.InlineKeyboardMarkup()
    for lst in lists:
        keyboard.add(types.InlineKeyboardButton(text=lst['name'], callback_data=f"map_list:{lst['id']}"))
...
async def map_list(callback_query: types.CallbackQuery, user: User):
    ...
    # ذخیره نقشه لیست
    map_list = await Map_List.find_one(Map_List.telegram_id == user.telegram_id, Map_List.trello_list_id == list_id)
    if not map_list:
        map_list = Map_List(telegram_id=user.telegram_id, trello_list_id=list_id, mapped_category=mapped_category)
    else:
